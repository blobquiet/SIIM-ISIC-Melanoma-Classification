...
#class LRCosineScheduler():
  #def __init__(self, optimizer, t_initial=20):
  ...
  #def __call__(self, value_loss):
    #self.lr_scheduler.step(value_loss)


class LRCosineScheduler():
  def __init__(self, optimizer, t_initial=20, warmup_t=0, warmup_lr_init=0, cycle_limit=20, cycle_decay=0.8):
    self.optimizer = optimizer
    self.t_initial = t_initial
    self.warmup_t = warmup_t
    self.warmup_lr_init = warmup_lr_init
